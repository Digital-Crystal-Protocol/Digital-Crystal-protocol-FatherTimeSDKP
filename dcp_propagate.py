...
SHA256_HASH = "4cfaaaa767a92418e2abbf209fe20117f94a2abc0aa9e93e22985bc12ecd24"
...
> **Smith, D. P. (2025). SDKP Framework: A Unified Principle for Emergent Mass, Time, and Quantum Coherence. Zenodo. [https://doi.org/](https://doi.org/){AUTH_DOI}**
...
def batch_propagate_authorship(directory_path):
    """
    Propagates the optimized authorship information to multiple files in a directory and its subdirectories.
    This function targets all files within all roots of the repository.
    """
    if not os.path.exists(directory_path):
        print(f"❌ Error: Directory {directory_path} does not exist.")
        return

    # Comprehensive Exclusion List for safety (prevents corruption of binaries, logs, and metadata)
    EXCLUDED_EXTENSIONS = [
        '.png', '.jpg', '.jpeg', '.gif', '.bin', '.exe', '.dll', '.zip', '.tar', '.gz', 
        '.pdf', '.docx', '.xlsx', '.sqlite', '.db', '.dat', '.json', '.yaml', '.yml', 
        '.ini', '.log', '.lock', '.svn', '.DS_Store', '.o', '.pyc', '.class', '.tmp', 
        '~', '.cff', '.gitignore', '.gitattributes', '.tex', '.md', '.sh', '.bash', '.bat',
        '.html', '.css', '.js', '.lock'
    ]
    ...
    for root, dirs, files in os.walk(directory_path):
        ...
        # CRITICAL: Skip hidden directories (like .git, .vscode, etc.)
        ...
        for file in files:
            ...
            # Skip the propagation script itself to prevent infinite loops/corruption
...
# ==============================================================================
# ENTRY POINT FOR REPOSITORY DEPLOYMENT
# The single call below executes the protocol on the current directory and all subdirectories.
# ==============================================================================
